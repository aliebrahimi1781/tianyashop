...
<div tools="${symbol_pound}tt" class="easyui-panel" title="权限设置" style="padding: 10px;" fit="true" border="false" id="function-panel"></div>
</div>
<div id="tt"></div>
</div>
<script type="text/javascript">
function setfunbyrole(id,roleName) {
	${symbol_dollar}("${symbol_pound}function-panel").panel(
		{
			title :roleName+":当前权限",
			href:"roleController.do?fun&roleId=" + id
		}
	);
	${symbol_dollar}('${symbol_pound}function-panel').panel("refresh" );
...
function delRole(id){
	...
	${symbol_dollar}.dialog.confirm('确定删除该记录吗', function(){
		doSubmit(url,tabName);
		rowid = '';
		${symbol_dollar}("${symbol_pound}function-panel").html("");//删除角色后，清空对应的权限
	}, function(){
	});
}
</script>
