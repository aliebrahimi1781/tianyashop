...
<div style="float: left;"><a href="${symbol_pound}" class="easyui-linkbutton" plain="true" icon="icon-add" onclick="add('部门录入','departController.do?add','departList')">部门录入</a> <a href="${symbol_pound}"
...
function queryUsersByRowData(rowData){
	if(li_east == 0){
	   ${symbol_dollar}('${symbol_pound}main_depart_list').layout('expand','east'); 
	}
	${symbol_dollar}('${symbol_pound}userListpanel').panel("refresh", "departController.do?userList&departid=" + rowData.id);
}
...
function queryUsersByDepart(departid){
	...
	${symbol_dollar}('${symbol_pound}userListpanel').panel("refresh", "departController.do?userList&departid=" + departid);
}
//-->
</script>