...
<html>
 ...
 <body style="overflow-y: hidden" scroll="no">
  <t:formvalid formid="messagefrom" dialog="true" usePlugin="password" layout="table" action="autoResponseController.do?doSave">
   ...
   <input id="templateName" name="templateName" type="hidden" value="${symbol_dollar}{templateName}"/>
   <table style="width:600px;" cellpadding="0" cellspacing="1" class="formtable">
    <tr>
     ...
     <td class="value">
       ...
       <input id="keyWord" class="inputxt" name="keyWord"  value="${symbol_dollar}{keyWord}" datatype="*" nullmsg="关键字不能为空！">
    ...
    <tr>
     ...
     <td class="value">
          <select name="msgType" id="msgType">
          		<option value="text"  <c:if test="${symbol_dollar}{msgType=='text'}">selected="selected"</c:if>>文本消息</option>
          		<option value="news"  <c:if test="${symbol_dollar}{msgType=='news'}">selected="selected"</c:if>>图文消息</option>
          </select>
     </td>
    </tr>
     ...
     <td class="value" id="textId">
          <select name="resContent" id="textContent">
          		<c:forEach items="${symbol_dollar}{textList}" var="textVal">
          			<option value="${symbol_dollar}{textVal.id}" name="textMsg" <c:if test="${symbol_dollar}{resContent==textVal.id}">selected="selected"</c:if>>${symbol_dollar}{textVal.templateName}</option>
          		</c:forEach>
          </select>
     </td>
     <td class="value" style="display:none" id="newsId">
          <select name="resContent" id="newsContent" disabled="disabled">
          		<c:forEach items="${symbol_dollar}{newsList}" var="newsVal">
          			<option value="${symbol_dollar}{newsVal.id}" <c:if test="${symbol_dollar}{resContent==newsVal.id}">selected="selected"</c:if>>${symbol_dollar}{newsVal.templateName}</option>
          		</c:forEach>
          </select>
     </td>
    </tr>
