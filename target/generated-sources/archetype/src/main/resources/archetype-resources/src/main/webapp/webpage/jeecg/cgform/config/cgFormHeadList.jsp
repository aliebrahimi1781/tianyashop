...
<t:datagrid sortName="createDate" sortOrder="desc" name="tablePropertyList" title="智能表单配置" fitColumns="false"
	...
	<t:dgFunOpt exp="isDbSynch${symbol_pound}eq${symbol_pound}N" title="同步数据库" funname="doDbsynch(id,content)" />
	<t:dgFunOpt exp="isDbSynch${symbol_pound}eq${symbol_pound}Y&&jformType${symbol_pound}ne${symbol_pound}3" funname="addbytab(id,content)" title="表单模板"></t:dgFunOpt>
	<t:dgFunOpt exp="isDbSynch${symbol_pound}eq${symbol_pound}Y&&jformType${symbol_pound}ne${symbol_pound}3" funname="addlisttab(tableName,content)" title="功能测试"></t:dgFunOpt>
	<t:dgFunOpt exp="isDbSynch${symbol_pound}eq${symbol_pound}Y&&jformType${symbol_pound}ne${symbol_pound}3" funname="popMenuLink(tableName,content)" title="配置地址"></t:dgFunOpt>
...
<script type="text/javascript">
	...
	function delCgForm(id,name){
		${symbol_dollar}.dialog.confirm("确认删除该记录", function(){
			checkIsExit(id,name);
		}, function(){
		}).zindex();
	}
	...
	function checkIsExit(id,name){
		${symbol_dollar}.post("cgFormHeadController.do?checkIsExit&&name="+name,function(data){
			var d = ${symbol_dollar}.parseJSON(data);
			if (d.success) {
				${symbol_dollar}.dialog.confirm("表在数据库中已存在${symbol_escape}n确认删除", function(){
					delThis(id);
				}, function(){
				}).zindex();
			}else{
	...
	function remCgForm(id){
		${symbol_dollar}.dialog.confirm("确认移除该记录", function(){
			removeThis(id);
		}, function(){
		}).zindex();
	}
	...
	function updateForm(title,url,id){
		gridname=id;
		var rowsData = ${symbol_dollar}('${symbol_pound}'+id).datagrid('getSelections');
	...
	*/
	...
	function doMigrateOut(title,url,id){
		var rowData = ${symbol_dollar}('${symbol_pound}'+id).datagrid('getSelected');
		if (!rowData) {
			tip('请选择编辑项目');
			return;
		}
    ...
    <%--   update-end--Author:duanqilu  Date:20130910 for${symbol_pound}211 升级SQL导入导出--%>	
	//代码生成
	...
	function getListSelections(){
		var ids = '';
		var rows = ${symbol_dollar}("${symbol_pound}tablePropertyList").datagrid("getSelections");
