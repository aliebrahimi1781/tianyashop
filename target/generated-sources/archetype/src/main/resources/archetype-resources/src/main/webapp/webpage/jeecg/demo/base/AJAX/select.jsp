...
<script type="text/javascript">
	function getdemo(id) {
		if(id==''){
			${symbol_dollar}('${symbol_pound}demo').html("");
			return;
		}

		window.top.${symbol_dollar}.messager.progress({
			text : '正在加载数据....',
			interval : 300
		});
		...
		${symbol_dollar}.ajax({
			...
			success : function(data) {
				...
				if (d.success) {
					window.top.${symbol_dollar}.messager.progress('close');
					${symbol_dollar}('${symbol_pound}demo').html(d.msg);
	...
	<table class="formtable" cellpadding="0" cellspacing="1">
		<tr>
			...
			<td width="90%" class="value" colspan="3"><select style="width: 150px" name="proname" id="proname" onchange="getdemo(this.value);">
				...
				<c:forEach var="fun" items="${symbol_dollar}{funList}" varStatus="status">
					<option value="${symbol_dollar}{fun.id }">${symbol_dollar}{fun.functionName}</option>
				</c:forEach>
			</select></td>
		</tr>
		<tr>
