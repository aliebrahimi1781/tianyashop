...
#set( $symbol_dollar = '$' )
#set( $symbol_escape = '\' )
package ${package}.web.cgreport.util;
 ...
 * @version V1.0
 */
public class CgReportQueryParamUtil extends  ${package}.web.cgform.util.QueryParamUtil
	...
	public static void loadQueryParams(HttpServletRequest request, Map item, Map params) {
		...
		if("single".equals(queryMode)){
			...
			if(!StringUtil.isEmpty(value)){
				if(value.contains("*")){
					//模糊查询
					value = value.replaceAll("${symbol_escape}${symbol_escape}*", "%");
