...
<div class="Detail">
            <a class="ui-link" href="${symbol_pound}" id="opendialog" style="display: none;" data-rel="dialog"></a>
...
<div class="Detail">
<p>一等奖：${symbol_dollar}{hdEntity.priceone}  。奖品数量：${symbol_dollar}{hdEntity.onetotal} </p>
<p>二等奖：${symbol_dollar}{hdEntity.pricetwo} 。奖品数量：${symbol_dollar}{hdEntity.twototal} </p>
<p>三等奖：${symbol_dollar}{hdEntity.pricethree} 。奖品数量：${symbol_dollar}{hdEntity.threetotal} </p>
</div>
</div>
</div>
<div class="boxcontent boxyellow">
...
<div class="Detail">
<p>${symbol_dollar}{hdEntity.description} </p>
</div>
</div>
</div>
</div>
...
${symbol_dollar}(function(){
	window.requestAnimFrame=(
		...
		var now=0;
		var a=0.01;
		var hdId = "${symbol_dollar}{hdId}";
		var openId = "${symbol_dollar}{openId}";
		var isshow = 0;
			...
			if(now<steps.length){
				...
				setTimeout(function(){
					if(prize!=null){
						...
						if(isshow==0){
						    ${symbol_dollar}("${symbol_pound}prizetype").html(type);
							${symbol_dollar}("${symbol_pound}result").slideToggle(500);
							${symbol_dollar}("${symbol_pound}outercont").slideUp(500);
						}
		...
		i=10;
		${symbol_dollar}("${symbol_pound}inner").click(function(){
			if(running)
				return;
			if(count>=3){
				alert("您已经抽了 3 次奖。");
		...
		${symbol_dollar}("${symbol_pound}save-btn").bind("click",function(){
			var btn=${symbol_dollar}(this);
			var tel=${symbol_dollar}("${symbol_pound}tel").val();
			if(tel==''){
				alert("请输入手机号码");
				return
			}
			var regu=/^[1][0-9]{10}${symbol_dollar}/;
			...
			${symbol_dollar}.post('zpController.do?saveZpPrize',submitData,function(data){
