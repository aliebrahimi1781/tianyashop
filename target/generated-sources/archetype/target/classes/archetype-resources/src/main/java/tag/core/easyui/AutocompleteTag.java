...
public class AutocompleteTag extends TagSupport {
	...
	public StringBuffer end() {
		...
		.append("${symbol_dollar}(${symbol_escape}"${symbol_pound}"+name+"${symbol_escape}").autocomplete(${symbol_escape}""+dataSource+"${symbol_escape}",{")
		.append("max: 5,minChars: "+minLength+",width: 200,scrollHeight: 100,matchContains: true,autoFill: false,extraParams:{")
        .append("featureClass : ${symbol_escape}"P${symbol_escape}",style : ${symbol_escape}"full${symbol_escape}",	maxRows : "+maxRows+",labelField : ${symbol_escape}""+labelField+"${symbol_escape}",valueField : ${symbol_escape}""+valueField+"${symbol_escape}",")
		.append("searchField : ${symbol_escape}""+searchField+"${symbol_escape}",entityName : ${symbol_escape}""+entityName+"${symbol_escape}",trem: getTremValue}");
		...
		nsb.append("}); });")
		.append("function getTremValue(){return ${symbol_dollar}(${symbol_escape}"${symbol_pound}"+name+"${symbol_escape}").val();}")
        .append("</script>")
        .append("<input type=${symbol_escape}"text${symbol_escape}" id=${symbol_escape}""+name+"${symbol_escape}" datatype=${symbol_escape}""+datatype+"${symbol_escape}" nullmsg=${symbol_escape}""+nullmsg+"${symbol_escape}" errormsg=${symbol_escape}""+errormsg+"${symbol_escape}"/>");
		if(StringUtil.isNotEmpty(label)){
			nsb.append(" value="+label+" readonly=true");
		}
		nsb.append("<input type=${symbol_escape}"hidden${symbol_escape}" id=${symbol_escape}""+valueField+"${symbol_escape}" name=${symbol_escape}""+valueField+"${symbol_escape}"/>");
		return nsb;
	}
