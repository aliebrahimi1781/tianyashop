...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="huodongList${symbol_dollar}{type}" title="活动" actionUrl="huodongController.do?datagrid&type=${symbol_dollar}{type}" idField="id" fit="true">
  ...
  <script type="text/javascript">
  	...
  	function showRecord(id){
  	 ...
  	 function myadd(title,addurl,gname,width,height) {
 		gridname=gname;
 		var getData = ${symbol_dollar}('${symbol_pound}'+gridname).datagrid('getData');
 		if(getData.total!=0){
 			if("${symbol_dollar}{type}"==2){
 				tip('一个用户只能创建一个大转盘活动');
 			}else{
 				tip('一个用户只能创建一个刮刮乐活动');
 			}
