...
public class UploadUtils {
	...
	private String validateFields(HttpServletRequest request) {
		...
		if (contentType == null || !contentType.startsWith("multipart")) {
			// TODO
			${package}.core.util.LogUtil.info("请求不包含multipart/form-data流");
			errorInfo = "请求不包含multipart/form-data流";
		} else if (maxSize < contentLength) {
			// TODO
			${package}.core.util.LogUtil.info("上传文件大小超出文件最大大小");
	...
	private String saveFile(FileItem item) {
		...
		if (item.getSize() > maxSize) { // 检查文件大小
			...
			error = "上传文件扩展名是不允许的扩展名。${symbol_escape}n只允许" + extMap.get(dirName) + "格式。";
		} else {
			String newFileName;
			...
			try {
				...
				e.printStackTrace();
				${package}.core.util.LogUtil.info("上传失败了！！！");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
