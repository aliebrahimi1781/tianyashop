...
<t:datagrid name="jeecgDemoList" title="DEMO示例列表" autoLoadData="true" actionUrl="jeecgDemoController.do?datagrid" sortName="userName" fitColumns="true"
	...
	<t:dgFunOpt exp="status${symbol_pound}eq${symbol_pound}0" operationCode="szqm" funname="szqm(id)" title="审核" />
...
<script type="text/javascript">
	...
	function getListSelections(){
		var ids = '';
		var rows = ${symbol_dollar}("${symbol_pound}jeecgDemoList").datagrid("getSelections");
	...
	${symbol_dollar}(document).ready(function(){
		${symbol_dollar}("input[name='createDate_begin']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
		${symbol_dollar}("input[name='createDate_end']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
		${symbol_dollar}("input[name='birthday']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
	});
