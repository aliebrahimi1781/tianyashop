...
function typeGridTree_AddType() {
	...
	var node = ${symbol_dollar}("${symbol_pound}"+treeCtrlId).treegrid('getSelected');
	if (node == null) {
		tip("请选择一个字典组");
		return;
	}
	if (node.state == "closed" || node.children) {
		...
		node = ${symbol_dollar}("${symbol_pound}"+treeCtrlId).treegrid('getParent', node.id); //获取当前节点的父节点
	}
...
function typeGridTree_UpdateType() {
	...
	if (node.state == "closed" || node.children) {
		...
		var pnode = ${symbol_dollar}("${symbol_pound}"+treeCtrlId).treegrid('getParent', node.id); //获取当前节点的父节点
